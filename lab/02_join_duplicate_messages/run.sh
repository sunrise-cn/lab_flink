...
# 用法: ./run.sh [ORDER_NO]  （默认 T001；ORDER_NO 仅用于自定义单号，重跑无需换号）
set -euo pipefail
cd "$(dirname "$0")"

ORDER_NO="${1:-T001}"
JOB_NAME='lab02-join-duplicate-messages'   # 与 demo.sql 里的 SET 'pipeline.name' 对应
ORDER_SRC_TOPIC='02_join_duplicate_messages_order_src'
PROPS_SRC_TOPIC='02_join_duplicate_messages_props_src'
INNER_SINK_TOPIC='02_join_duplicate_messages_inner_sink'
LEFT_SINK_TOPIC='02_join_duplicate_messages_left_sink'
# 项目根的 ./lab 挂载到容器的 /opt/flink/lab
SQL_FILE="/opt/flink/lab/02_join_duplicate_messages/demo.sql"

echo '== 1/7 创建 topic（幂等）'
for T in "$ORDER_SRC_TOPIC" "$PROPS_SRC_TOPIC" "$INNER_SINK_TOPIC" "$LEFT_SINK_TOPIC"; do
...
../../scripts/purge-topic.sh "$ORDER_SRC_TOPIC" "$PROPS_SRC_TOPIC" "$INNER_SINK_TOPIC" "$LEFT_SINK_TOPIC"
...
echo '== 6/7 发送测试消息'
./send-messages.sh "$ORDER_NO"

echo '== 7/7 消费 sink topic（value 为 null 的是回撤墓碑）'
# 注意：macOS bash 3.2 下 $VAR 后紧跟全角字符会被误并入变量名，必须加 {} 隔开
echo "-- INNER JOIN 分支（${INNER_SINK_TOPIC}）"
../../scripts/consume.sh "$INNER_SINK_TOPIC"
echo "-- LEFT JOIN 分支（${LEFT_SINK_TOPIC}）"
../../scripts/consume.sh "$LEFT_SINK_TOPIC"
