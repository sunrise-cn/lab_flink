...
ORDER_NO="${1:-T001}"
JOB_NAME='lab01-leftjoin-inner-join'   # 与 demo.sql 里的 SET 'pipeline.name' 对应
SRC_TOPIC="D-oneid-chat-offline-retry-msg-202410251049"
SINK_TOPIC="D-hanghaitu_supplier_service_role_split_inp-202608101417"
# 项目根的 ./lab 挂载到容器的 /opt/flink/lab
SQL_FILE="/opt/flink/lab/01_leftjoin_and_inner_join/demo.sql"

echo '== 1/7 创建 topic（幂等）'
docker exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 \
  --create --if-not-exists --topic "$SRC_TOPIC" --partitions 1 --replication-factor 1 2>/dev/null
docker exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 \
  --create --if-not-exists --topic "$SINK_TOPIC" --partitions 1 --replication-factor 1 2>/dev/null
...
../../scripts/purge-topic.sh "$SRC_TOPIC" "$SINK_TOPIC"
